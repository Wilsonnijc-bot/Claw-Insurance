...
Flow:
1. The Docker entrypoint starts this server on port 3456.
...
class LauncherServer:
    ...
    async def stop(self) -> None:
        ...
        if self._bridge_proc:
            try:
                from nanobot.runtime.gateway_runtime import stop_whatsapp_bridge
                stop_whatsapp_bridge(self._bridge_proc)
            except Exception:
                pass
    ...
    async def _start_gateway(self) -> None:
        ...
        try:
            ...
            self._bridge_proc = start_whatsapp_bridge(config)
            self._privacy_proc = maybe_enable_privacy_gateway(config)
