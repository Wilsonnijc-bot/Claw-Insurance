...
CDP_HELPER_PREREQ_DETAIL = (
    "Host Chrome/CDP helper is not reachable. "
    "Install/start the host CDP helper first, then restart Docker Compose."
)
...
class ApiServer:
    ...
    def _whatsapp_helper_install_command(platform_name: str) -> str:
        return CDP_HELPER_PREREQ_DETAIL
    ...
    def _get_whatsapp_sync_status(self) -> dict[str, Any]:
        ...
        if browser_mode == "cdp" and running_in_docker_workspace:
            if not helper_url:
                logger.warning(CDP_HELPER_PREREQ_DETAIL)
                return {
                    "available": False,
                    "message": (
    ...
    def _restart_bridge_sync(self) -> dict[str, str]:
        ...
        while time.time() < deadline:
            ...
            if whatsapp_bridge_running(self.config):
