"""Windows helper that launches or reuses a host Chrome CDP window."""
...
HELPER_LABEL = _common.WINDOWS_HELPER_LABEL


def helper_install_dir():
    return _common.helper_install_dir("windows")


def helper_python_script_path():
    return _common.helper_python_script_path("windows")


def helper_launcher_script_path():
    return _common.helper_launcher_script_path("windows")


def helper_stdout_log_path():
    return _common.helper_stdout_log_path("windows")


def helper_stderr_log_path():
    return _common.helper_stderr_log_path("windows")


def helper_token_path():
    return _common.helper_token_path("windows")


def windows_task_name() -> str:
    return _common.windows_task_name()


def load_helper_token() -> str:
    return _common.load_helper_token("windows")


def write_helper_token(token: str):
    return _common.write_helper_token(token, platform_name="windows")


def load_or_create_helper_token() -> str:
    return _common.load_or_create_helper_token("windows")


def resolve_chrome_path(configured_path: str = ""):
    return _common.resolve_chrome_path(configured_path, platform_name="windows")
...
def install_windows_helper(helper_token: str = "", frozen_executable: str = "") -> dict[str, str]:
    return _common.install_windows_helper(
...
def build_cli_parser():
    return _common.build_cli_parser("Nanobot Windows host CDP helper")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "install":
        try:
            result = install_windows_helper(helper_token=str(args.helper_token or "").strip())
        except Exception as exc:
            print(f"Failed to install the Windows host CDP helper: {exc}", file=sys.stderr)
            return 1
        if args.json:
            print(json.dumps(result, ensure_ascii=False))
        else:
            print(f"Installed Windows host CDP helper at {result['helper_url']}")
    ...
    if args.command == "health":
        ...
        if healthy:
            print(f"Windows host CDP helper is healthy at {args.helper_url}")
            return 0
        print(f"Windows host CDP helper is not reachable at {args.helper_url}", file=sys.stderr)
        return 1
