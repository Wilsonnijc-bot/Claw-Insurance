...
from nanobot.api.server import ApiServer, CDP_DRAFT_DISABLED_DETAIL, CDP_HELPER_PREREQ_DETAIL
...
async def test_status_exposes_sync_availability_without_bridge_error(
    ...
    assert CDP_HELPER_PREREQ_DETAIL in json.loads(response.text)["whatsapp_sync_message"]


def test_whatsapp_docker_up_hint_uses_platform_specific_commands() -> None:
    assert ApiServer._whatsapp_docker_up_hint("macos") == "docker compose up -d --build"
    assert ApiServer._whatsapp_docker_up_hint("linux") == "docker compose up -d --build"
    assert ApiServer._whatsapp_docker_up_hint("windows") == "docker compose up -d --build"


def test_whatsapp_helper_install_command_uses_lightweight_platform_modules() -> None:
    assert ApiServer._whatsapp_helper_install_command("macos") == CDP_HELPER_PREREQ_DETAIL
    assert ApiServer._whatsapp_helper_install_command("linux") == CDP_HELPER_PREREQ_DETAIL
    assert ApiServer._whatsapp_helper_install_command("windows") == CDP_HELPER_PREREQ_DETAIL
