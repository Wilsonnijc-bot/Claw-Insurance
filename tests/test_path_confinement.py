...
class TestDockerConfinement:
    ...
    def test_compose_uses_docker_runtime_entrypoint(self):
        compose = project_root() / "docker-compose.yml"
        if not compose.exists():
            pytest.skip("docker-compose.yml not found")
        content = compose.read_text(encoding="utf-8")
        dockerfile = (project_root() / "Dockerfile").read_text(encoding="utf-8")
        assert "nanobot.runtime.docker_entrypoint" in dockerfile
        assert 'command: ["--api-port", "3456"]' in content
        assert 'command: ["launcher", "--api-port", "3456"]' not in content
        assert "pull_policy: always" not in content
        assert "~/.nanobot" not in content

    def test_compose_backend_runs_docker_entrypoint_args(self):
        compose = project_root() / "docker-compose.yml"
        if not compose.exists():
            pytest.skip("docker-compose.yml not found")
        content = compose.read_text(encoding="utf-8")
    ...
    def test_docker_up_script_removed(self):
        assert not (project_root() / "docker-up").exists()
